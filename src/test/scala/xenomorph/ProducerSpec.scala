...
class ProducerSpec extends UnitSpec with EmbeddedKafkaBuilder {
  ...
  "Producer should" - {
    "produce events that can be consumed" in {
      ...
      val sink = dataP to producer[String](cfg = producerConfig,
        topic = topic,
      ...
      val records = consumer.createMessageStreamsByFilter[String, String](Whitelist(topic), 1, new StringDecoder, new StringDecoder).flatMap { stream =>
