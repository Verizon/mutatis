...
import scalaz.stream.{Sink, merge, time}
...
object ExampleProducer extends App {
  ...
  val sink: Sink[Task, Duration] = xenomorph.producer(producerCfg, topic, keyEncoder, encoder)
...
object ExampleConsumer extends App {
  ...
  val consumer = merge.mergeN(streams)(
    ...
    xenomorph.consumer(consumerCfg, topic, keyDecoder, decoder, streams).map(stream => stream.evalMap { event =>
...
object worker {
  ...
  def process(event: DecodedEvent[Int,String]): Task[Unit] = Task.delay {
    Thread.sleep(250)
    println(s"thread=${Thread.currentThread.getName} topic=${event.messageAndMetadata.topic} " +
